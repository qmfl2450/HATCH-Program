...
router.get('/', async (req, res) => {
  ...
  const users = await service.userList(offset);
...
router.post('/signin', async (req, res) => {
  ...
  const user = await service.userFindById(name)
  if (!user) {
...
router.get('/:id', async (req, res) => {
  const {id} = req.params
  const user = await service.userInformation(id)
...
router.patch('/:id', async (req, res) => {
  const {id} = req.params
  const {pw} = req.body
  const edit_salt = getSalt()
  const encyp_pw = getHash(pw, edit_salt);
  const user = await service.updateUserInfo(id, encyp_pw, edit_salt)
  // const user = USER_DATA.find(user => {
  //   if (user.enable === 'disable') return false
  //   return user.id === id
  // })
...
router.delete('/:id', async (req, res) => {
  const {id} = req.params
  const enabled_status = 'disable'
  const user = await service.disabledUser(id, enabled_status)
  // const user = USER_DATA.find(user => {
  //   if (user.enable === 'disable') return false
  //   return user.id === id
  // })
  ...
  res.status(204).send()
  res.json({message: '유저 삭제에 성공하셨습니다'})
})
